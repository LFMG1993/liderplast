...
import type { Product, ProductCreationData } from '../../services/productService';
...
export function ProductForm({ isOpen, onClose, onSave, productToEdit, attributes, categories, isSubmitting }: ProductFormProps) {
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl" onClick={(e) => e.stopPropagation()}>
                <form onSubmit={handleSubmit}>
                    <div className="flex justify-between items-center p-6 border-b">
                        <h3 className="text-lg font-medium text-gray-900">{productToEdit ? 'Editar Producto' : 'Crear Producto'}</h3>
                    ...
                    <div className="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value={formData.name} onChange={handleInputChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-gray-700 p-2.5" required />
                        </div>
                        <div>
                            <label htmlFor="categoryId" className="block text-sm font-medium text-gray-700">Categoría</label>
                            <select name="categoryId" id="categoryId" value={formData.categoryId} onChange={handleInputChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-gray-700 p-2.5 focus:border-liderplast-primary focus:ring-liderplast-primary" required>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" value={formData.description} onChange={handleInputChange} rows={3} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-gray-700 p-2.5 focus:border-liderplast-primary focus:ring-liderplast-primary"></textarea>
                        </div>
                        <div className="flex items-center">
                            <input id="isFeatured" name="isFeatured" type="checkbox" checked={formData.isFeatured} onChange={handleInputChange} className="h-4 w-4 text-primary rounded border-gray-300 focus:ring-primary" />
                            <label htmlFor="isFeatured" className="ml-2 block text-sm text-gray-900">Marcar como producto destacado</label>
                        </div>

                        {/* Sección de Variantes */}
                        <div className="space-y-4 pt-4 border-t">
                            <h4 className="text-md font-medium text-gray-700 p-2.5">Variantes</h4>
                            {formData.variants.map((variant, index) => (
                                <div key={index} className="p-4 border rounded-md relative bg-gray-50 space-y-3">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <input type="text" placeholder="SKU (auto-generado)" value={variant.sku} onChange={(e) => handleVariantChange(index, 'sku', e.target.value)} className="w-full rounded-md border-gray-300 shadow-sm text-gray-700 p-2.5 focus:border-liderplast-primary focus:ring-liderplast-primary" required />
                                        <input type="number" placeholder="Precio" value={variant.price} onChange={(e) => handleVariantChange(index, 'price', parseFloat(e.target.value))} className="w-full rounded-md border-gray-300 shadow-sm text-gray-700 p-2.5 focus:border-liderplast-primary focus:ring-liderplast-primary" required />
                                        <input type="number" placeholder="Stock" value={variant.stock} onChange={(e) => handleVariantChange(index, 'stock', parseInt(e.target.value, 10))} className="w-full rounded-md border-gray-300 shadow-sm text-gray-700 p-2.5 focus:border-liderplast-primary focus:ring-liderplast-primary" required />
                                    </div>
                                    ...
                                    {formData.variants.length > 1 && (
                                        <button type="button" onClick={() => removeVariant(index)} className="absolute -top-2 -right-2 bg-white rounded-full p-0.5">
                                            <Trash2 className="h-4 w-4 text-red-500" />
                                        </button>
                                    )}
                                </div>
                            ))}
                            <Button type="button" variant="secondary" size="sm" onClick={addVariant}><PlusCircle className="h-4 w-4 mr-2" />Añadir Variante</Button>
                        </div>
                    </div>
